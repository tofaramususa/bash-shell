...
/*   Updated: 2023/08/12 21:35:02 by tmususa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool check_sytnax(t_token *headToken)
{
    ...
    //return something
    temp = headToken;
    if(temp && temp->type == PIPE) //NOTE that a redirection can be at the start but pipe cant
	{
        return (false);	
	}
    ...
    {
        if (temp->type == REDIR && temp->next->type != WORD) //there has to be word after a redirection or pipe else error
