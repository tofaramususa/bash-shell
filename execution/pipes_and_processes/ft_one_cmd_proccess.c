...
*/
int	pipex_one_cmd(t_command *av, t_shell *proc, char **envp) //takes the simple commands, shell data, and envp variables
{
	proc->scommand_index = 0;
	proc->env_vars = envp;
	if (av[0].cmd && ft_strcmp(av[0].cmd, "cd") == 0) //if cd then perform 
		return (do_operation(proc, av), ft_cd(av, proc)); //we do all necessary redirections then call the builin cd function
	else if (av[0].cmd && ft_strcmp(av[0].cmd, "exit") == 0) //else if exit has been typed
	{
		do_operation(proc, av); //perform rediractions
		ft_exit(av, proc); //call exit function
		return (1);
	}
	else if (av[0].cmd && ft_strcmp(av[0].cmd, "unset") == 0) //perform the redir
